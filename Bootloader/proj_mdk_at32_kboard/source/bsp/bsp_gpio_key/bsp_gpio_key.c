...
void bsp_key_gpio_init()
{
  ...
  gpio_init_struct.gpio_drive_strength = GPIO_DRIVE_STRENGTH_STRONGER; /* 设置引脚驱动强度为较强 */
  gpio_init_struct.gpio_out_type  = GPIO_OUTPUT_PUSH_PULL; /* 设置输出类型为推挽输出 */
  gpio_init_struct.gpio_mode = GPIO_MODE_INPUT; /* 设置引脚模式为输入模式 */
  gpio_init_struct.gpio_pull = GPIO_PULL_UP; /* 设置引脚为上拉模式 */


  gpio_init_struct.gpio_pins = GPIO_PINS_0 | GPIO_PINS_9 | GPIO_PINS_10 | GPIO_PINS_11 |
                               GPIO_PINS_12  | GPIO_PINS_15;
  gpio_init(GPIOA, &gpio_init_struct); /* 初始化 GPIOA 端口 */
  ...
  gpio_init(GPIOB, &gpio_init_struct); /* 初始化 GPIOB 端口 */
  ...
  gpio_init(GPIOC, &gpio_init_struct); /* 初始化 GPIOC 端口 */
  
	gpio_init_struct.gpio_pins = GPIO_PINS_2;
  gpio_init(GPIOD, &gpio_init_struct); /* 初始化 GPIOD 端口 */

  gpio_init_struct.gpio_pins = GPIO_PINS_6 | GPIO_PINS_7;
  gpio_init(GPIOF, &gpio_init_struct); /* 初始化 GPIOF 端口 */
}


static uint8_t read_key(uint8_t id)
{
    switch(id)
    {
        // KEY1到KEY36
